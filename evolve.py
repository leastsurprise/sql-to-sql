...
def migrate_as_per_json(txt):
    ...
    for next_func in func_map:
        ...
        for x in reversed(b2f):
            ...
            this_tag = ''
            if len(tag_text) > 0 and not ("SUPPRESS_TAG" in func_map[next_func] and func_map[next_func]["SUPPRESS_TAG"] == 'Y'):
...
def print_command_line_options():
    ...
    print('evolve.py -i <inputpath> -o <outputpath> -t <tag text> -q <only allowable string quote character> -x <name_of_ignore_list_file>')
    ...
    print('                              [The optionnal <name_of_ignore_list_file> is similar to .gitignore')
...
if __name__ == '__main__':
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:],"hli:o:t:q:x:")
    ...
    for opt, arg in opts:
        ...
        if opt == '-l':
            ...
            dotignorefile = arg.strip()
    if no_option_given:
        print_command_line_options()
        sys.exit(0)
    if dotignorefile != '':
        if os.path.isfile(dotignorefile):
            files_to_ignore = (open(dotignorefile, "rt", encoding='utf-8')).read().splitlines()
        else:
            print('Option -x is not a file.  THis is your ignore list - it needs to be a text file.')
            print_command_line_options()
            sys.exit(2)
    if inputpath != '':
        try:
            if os.path.isdir(inputpath) and (os.path.isdir(outputpath) or list_functions == True):
                for input_file in [f for f in os.listdir(inputpath) if os.path.isfile(os.path.join(inputpath, f)) and re.match('.*(sas|sql)$', f, flags=re.IGNORECASE)]:
                    if input_file in files_to_ignore:
                        continue
                    if list_functions:
                        file_queue.append({
